...
# ----------------- 1. LOAD DATA (تعديل السطر 38 للسلامة) -----------------
@st.cache_data
def load_data():
    try:
        ...
        # تحويل الأعمدة لنصوص قبل الـ strip والـ upper عشان نمنع الـ AttributeError
...
if query:
    ...
    col2.metric("Radio (BC) in Selection", len(f_df[f_df['station_class'] == 'BC']))
    col3.metric("TV (BT) in Selection", len(f_df[f_df['station_class'] == 'BT']))

    # ---------------- 4. Voice Logic (Stable) ----------------
    if final_count == 0:
    ...
    # ---------------- 5. Chart (Native Streamlit - البديل الأسرع لـ Matplotlib) ----------------
    st.subheader("📊 Station Class Distribution")
    chart_data = f_df['station_class'].value_counts()
    st.bar_chart(chart_data)

    # ---------------- 6. MAP (Fixing Rerender) ----------------
    ...
    if not map_df.empty:
        ...
        # إضافة key ثابت عشان الخريطة متهنجش مع الـ Audio
        st_folium(m, width=1000, height=500, key="main_map")
    else:
        st.info("No coordinates to display for this filter.")
