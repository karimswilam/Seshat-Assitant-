...
ISO_MAP = {
    'EGY': 'eg', 'ARS': 'sa', 'TUR': 'tr', 'CYP': 'cy', 'GRC': 'gr', 'ISR': 'il', 'USA': 'us', 'F': 'fr', 'D': 'de', 'I': 'it', 'G': 'gb', 'RUS': 'ru', 'UAE': 'ae', 'JOR': 'jo', 'LBN': 'lb', 'QAT': 'qa', 'KWT': 'kw', 'OMA': 'om', 'BHR': 'bh', 'IRQ': 'iq', 'SDN': 'sd', 'LBY': 'ly', 'TUN': 'tn', 'ALG': 'dz', 'MRC': 'ma'
}
...
# --- ENHANCED SCROLLING FLAG LIST (As Requested) ---
st.write("🌍 **Quick Country Lookup:**")

# إنشاء شريط التمرير الأفقي
st.markdown('<div class="scroll-wrapper">', unsafe_allow_html=True)
# سنقوم بعرض الدول المهمة أولاً أو كل الدول
for adm_code, adm_name in ADM_LIST.items():
    iso = ISO_MAP.get(adm_code, adm_code[:2].lower())
    flag_url = f"https://flagcdn.com/w80/{iso}.png"
    
    # نضع كل دولة في عمود صغير داخل الـ Scroll
    with st.container():
        # استخدام Markdown لعرض الصورة والاسم فوق الزر
        if st.button(f"🔍 {adm_code}", key=f"btn_{adm_code}", help=adm_name):
            st.session_state.query_input = f"{adm_name} comparison"
            st.rerun()
        st.image(flag_url, width=50)
        st.caption(adm_name[:10])
