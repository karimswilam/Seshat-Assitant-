...
st.markdown("""
    ...
    /* تنسيق السلايسر */
    div[data-testid="stExpander"] div[role="button"] p { font-weight: bold; color: #1E3A8A; }
    .centered-msg { 
...
# --- NEW: CHIC SIDEBAR SLICER ---
st.sidebar.markdown("### 📊 Quick Slicer")
with st.sidebar.expander("🌍 Select Countries (ADM)", expanded=True):
    # تحويل مفاتيح COUNTRY_DISPLAY لقائمة منسقة للسلايسر
    slicer_options = [f"{v['en']} | {v['ar']}" for k, v in COUNTRY_DISPLAY.items()]
    selected_slicer = st.multiselect("Filter by Country:", options=slicer_options)

# --- Logic: Link Slicer to Input ---
slicer_query = ""
if selected_slicer:
    # استخراج أسماء الدول بالانجليزية فقط للبحث
    slicer_query = " ".join([s.split(" | ")[0] for s in selected_slicer])
...
with st.container(border=True):
    ...
    # تحديد القيمة الافتراضية للـ input (صوت أولاً، ثم سلايسر، ثم فارغ)
    input_val = ""
    if voice_raw:
        input_val = speech_to_text_robust(voice_raw)
    elif slicer_query:
        input_val = slicer_query
        
    with col_v2:
        # جعل القيمة مرتبطة بالـ Slicer أو الـ Voice
...
if query and db is not None:
    ...
    if not success:
        ...
        if len(reports) == 1:
            ...
            # Multi-Country Comparison (Logic as per your code)
